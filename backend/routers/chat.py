...
def _message_payload(message: ChatMessage, sender_name: str = "Unknown") -> dict:
    return {
        ...
        "created_at": _aware_utc(message.created_at).isoformat(),
        "updated_at": _aware_utc(message.updated_at).isoformat() if getattr(message, "updated_at", None) else None,
        "edited_at": _aware_utc(message.edited_at).isoformat() if getattr(message, "edited_at", None) else None,
...
async def get_chat_rooms(current_user: User = Depends(get_current_user)):
    ...
    for room in rooms:
        ...
        rd = {
            ...
            "last_message_at": _aware_utc(room.last_message_at).isoformat() if room.last_message_at else None,
            ...
            if action == "send_message":
                ...
                broadcast = {
                    ...
                    "data": {
                        ...
                        "sender_name": sender_name,
                        "content": content,
                        "created_at": _aware_utc(new_msg.created_at).isoformat(),
                        "updated_at": _aware_utc(new_msg.updated_at).isoformat() if new_msg.updated_at else None,
                        "edited_at": None,
                        "is_read": False,
